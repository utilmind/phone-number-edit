...
// OBJECT HELPERS
function valOf(obj, key, def) { // unfortunately object.valueOf() doesn't returns false if object has no property. So we need custom func.
...
function ieEvent(targetObj, // window, document, etc
                 eventName, // string
                 params) {  // any type
    var event;
...
String.prototype.gap = function(
  ...
  if (0 < pos) // if separator found and first item is not empty
    return { 0: me.substr(0, pos),
             1: me.substr(pos + separator.length, me.length) };

  if ("undefined" === typeof useEmpty) // Don't use strict comparison!
    return false;
...
function floatRound(v, precise) { // if precise is NEGATIVE (or any non-numeric value for default precision) will be used an exact fixed number of digits after floating point.
    var isFixed = false;
...
function uniqId() { // generator of unique Ids. (Can't use uniqueId() because of naming conflict with jQuery UI.)
  var token = "_uniqId_",
      u = window[token]; // using global is bad practice :(( Think of something better. Maybe jQuery's .data()?

  return window[token] = ("undefined" === typeof u ? 0 : ++u);
}
...
function umboxPrep(t, options) { // isBold, icon, iconWidth
  var icon;
...
function umconfirm(t, onOk, options) { // onOk, onCancel, icon, is_bold, icon_width
...
function restoreSubmit($form) {
    var $submitBtn = $($form).find(":submit").filter(":disabled"); // don't do anything if it's already enabled.
    if ($submitBtn.length) {
        var title = $submitBtn.data("title");
        $submitBtn.prop("disabled", false);
        if ("INPUT" === $submitBtn.prop("tagName").toUpperCase())
            $submitBtn.val(title);
        else
            $submitBtn.html(title);
    }
}

// Submit <textarea>'s, execute some submit function on Ctrl+Enter.
// USAGE: <textarea onKeyPress="return ctrlEnter(event, function(){ $(event.target).closest('form').submit() })"
// UPD: if fn is not function, it considered as the form, which should be submitted with jQuery.
function ctrlEnter(e, fn) {
    if (fn) {
        var key = e.keyCode;
...
// SHARING
function shareUrl(serviceUrl/* anchor object allowed */, data /* parameters should be plain, data must be encoded. Allowed #id to some meta property. */, socNetwork) {
  var e, curUrl = "";
...
function isBSSmall(mode) {
    if (!mode) mode = getBSMode(mode);
    return "sm" === mode || "xs" === mode;
}

function hideBSTooltips() {
    $(".tooltip:visible").tooltip("hide");
}

function initBSTooltips($scope, customSelector) { // correct $scope required here. We don't check its availability
    // tooltips
...
function initDeviceTypeClass($scope) { // first used on GoldHorn Records
...
function initCommonBehaviors($scope) {
    ...
    $scope.find("input.click-select").on("focus", function() {
       ...
       try { // try..catch because of possible error with me.select() in IE
           me.select();
           setTimeout(function() { // Edge hiding selection. Let's select it again after short timeout.
...
function flashItem(el, flashClass) {
  if ((el = $(el)).length) { // we don't care if the element not exists, but certainly don't want an error
    ...
    if (!isNaN(duration) || (duration.indexOf("ms") < 0)) // duration in seconds? Convert to milliseconds.
...
function headExtResource(url, // can be either an URL (string type), so new object will be created OR you may specify an object (HTML element) which *already created*. Object can be outside of the DOM, not necessary attached to the "document".
...
function loadScript(scriptUrl, // array ["url1", "url2", "url3", ...] is allowed. If array specified, all urls will triggerer onLoad one by one, in the same order how they has been specified.
...
function fillHtml(container,  // ID (string) with # prefix, OR an object within DOM. If ID is string, it will unwrap the received the content under that ID.
                  ...
                  showOptions,// set to TRUE to unhide(show)+enable the container.
                              // -1 or any negative value means that data can be APPEND to the "document.body" if the container not found at all.
                  moveObjToHead) { // which external objects should be moved to <head> section. If NULL (not specified), DEFAULT value will be used (for <scripts> and <links> to CSS).
  ...
  var isString = "string" === typeof $data,
      doCheckExtResources = !isString || ($data.indexOf(">") > 0); // otherwise we consider it as prepared jQuery object. Object resourses contains some tags in any case.
  ...
  if (doCheckExtResources) { // has tags?
    ...
    if ("undefined" === typeof moveObjToHead) // set default value
      moveObjToHead = 'script[src], link[rel="stylesheet"][href], link[rel="preload"][as="style"][href]';
    ...
    $data.find(moveObjToHead).each(function() {
    ...
    if (arrDeferQueue.length)
      document.uLateLoad = 1; // signal to start listening document.addEventListener("uLateLoad", ...) in wReady().
...
function getUrlParam(param, url, getIntId) { // if url is empty, "window.location.search" is used
...
function setUrlParam(key, val, // if val is undefined, it will be removed from URL
    ...
    var newParams,
        ...
        isAdd = ("undefined" !== typeof val) && ("" != val); // Don't check val as boolean, because numerical 0 considered as good value.

    if (isAdd)
        val = encodeURIComponent(val);
    key = encodeURIComponent(key);

    if ("" == params) // don't check with "===", it can be boolean.
...
function commonHTTPError(httpStatusCode, url, responseData, errorTitle, errorMsg) {
...
function getURL(url, onResponse, showHourglass, useCache, noLang, headers, postData /* if specified, this is POST request, not GET */) {

  var ireq = XMLHttpRequest ? new XMLHttpRequest() : (ActiveXObject /*IE6*/ ? new ActiveXObject("Microsoft.XMLHTTP") : false),
  ...
  if (ireq) {
    try {
      if (showHourglass) {
        if ("undefined" === typeof hourglass) showHourglass = 0;
        else hourglass.show(1);
      }
      if (onResponse)
      ...
      if (!noLang && ("undefined" !== typeof lang) && (0 > url.indexOf(langToken))) // add both to POST and GET requests
        postData+= langToken + lang;

      if (("undefined" !== typeof psid) && (0 > url.indexOf(phpSessionIdToken)))
...
// IMPORTANT: onResponse does not occurs in case of HTTP error! Only alert message will be displayed. Use another tools to process error responses!
function postURL(url, postData, onResponse, showHourglass, headers, noLang) {
...
function timedPromise(checkFn, timeoutMs, attemptsCnt) {
  ...
  return new Promise(function(resolve, reject) {
      var checkCounter = 0,
          timerId = setInterval(function() {
            if (checkCounter >= attemptsCnt) {
              clearInterval(timerId);
              return reject(new Error(printf("Not initialized in {0} attempts.", attemptsCnt)));
            }
...
(function() {
  var extendJQuery = function() {
        ...
        // now extend jQuery. Caution! jQuery and $ can be not the same, especially in WordPress. We use $.
        $.fn.extend({
...
function requestRecaptcha3($form, recaptchaClientKey, onReady) {
  $form = $($form); // for sure

  var isRecaptchaReady = function() {
      ...
      applyRecaptcha = function() {
        grecaptcha.ready(function() {
          grecaptcha.execute(recaptchaClientKey, {action: "homepage"}).then(function(token) {
             if (!isRecaptchaReady()) { // maybe it's has been added by some other thread?
